...
class TitatitRobot(BaseTask):
    ...
    def _create_envs(self):
        ...
        """
        asset_path = self.cfg.asset.file.format(LEGGED_GYM_ROOT_DIR=ROOT_DIR)
        ...
        if self.cfg.domain_rand.randomize_lag_timesteps and self.cfg.domain_rand.lag_timesteps > 1:
            ...
            self.num_envs_indexes = list(range(0,self.num_envs))
            lag = max(0, self.cfg.domain_rand.lag_timesteps - 1)
            self.randomized_lag = [lag for i in range(self.num_envs)]
            denominator = max(1, self.cfg.domain_rand.lag_timesteps - 1)
            self.randomized_lag_tensor = torch.FloatTensor(self.randomized_lag).view(-1,1)/denominator
            self.randomized_lag_tensor = self.randomized_lag_tensor.to(self.device)
            self.randomized_lag_tensor.requires_grad_ = False
    ...
    def compute_observations(self):
        # 使用 clone() 创建用于观测的副本，绝不能直接修改 self.dof_pos 物理张量
        dof_pos_obs = (self.dof_pos - self.default_dof_pos).clone()
        dof_pos_obs[:, [3, 7, 11, 15]] = 0.0 # 掩蔽轮子观测值

        obs_buf =torch.cat((self.base_ang_vel  * self.obs_scales.ang_vel,
                            self.projected_gravity,
                            self.commands[:, :3] * self.commands_scale,
                            self.reindex(dof_pos_obs * self.obs_scales.dof_pos),
        ...
        if self.cfg.terrain.measure_heights:
            ...
            self.privileged_obs_buf = torch.cat([obs_buf, heights, priv_latent, self.obs_history_buf.view(self.num_envs, -1)], dim=-1)
        else:
            self.privileged_obs_buf = torch.cat([obs_buf, priv_latent, self.obs_history_buf.view(self.num_envs, -1)], dim=-1)

        self.obs_buf = self.privileged_obs_buf
    ...
    def _prepare_cost_function(self):
        ...
        self.cost_buf = torch.zeros(self.num_envs, len(self.cost_functions), dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _reward_dof_pos_limits(self):
        ...
        out_of_limits[:, [3, 7, 11, 15]] = 0.0 # 排除轮子的关节限位惩罚
    ...
    def _reward_stand_still(self):
        ...
        error[:, [3, 7, 11, 15]] = 0.0 
        return torch.sum(torch.abs(error), dim=1) * (torch.norm(self.commands[:, :2], dim=1) < 0.1)

    def _reward_wheel_zero_vel(self):
        # 惩罚在零指令（原地站立）时的轮子转动，起到“电子手刹”的作用
        is_standing = torch.norm(self.commands[:, :2], dim=1) < 0.1
        wheel_vel = self.dof_vel[:, [3, 7, 11, 15]]
        return torch.sum(torch.square(wheel_vel), dim=1) * is_standing
    ...
    def _reward_perceptive_obstacle_avoidance(self):
        # 惩罚机器人前方存在高障碍物的情况。
        # self.measured_heights (来自 _get_heights) 是地形相对于世界坐标系的高度。
        # self.root_states[:, 2] 是机器人基座的当前高度。
        
        # 计算地形相对于机器人基座的高度。
        # 正值表示地形高于基座（障碍物），负值表示低于基座（坑洼）。
        terrain_relative_to_base = self.measured_heights - self.root_states[:, 2].unsqueeze(1)
        
        # 定义一个“安全间隙”阈值。如果地形相对于基座的高度高于此阈值，则视为障碍物。
        # 例如，如果地形距离基座不到 0.1 米（即相对高度大于 -0.1m），则视为障碍物。
        obstacle_clearance_threshold = -0.1 # 米。如果地形相对基座高度 >= -0.1m，则视为障碍物。
        
        # 筛选机器人前方的采样点。
        x_coords = self.height_points[:, :, 0]
        y_coords = self.height_points[:, :, 1]
        
        front_x_min = 0.2  # 机器人前方 0.2 米开始
        front_x_max = 0.8  # 机器人前方 0.8 米结束
        lateral_y_max = 0.3 # 机器人两侧 0.3 米范围内
        
        in_front_mask = (x_coords > front_x_min) & \
                        (x_coords < front_x_max) & \
                        (y_coords.abs() < lateral_y_max)
                        
        # 对前方高于阈值的地形施加惩罚。torch.relu 确保只惩罚正向的“障碍物”值。
        penalty_values = torch.relu(terrain_relative_to_base * in_front_mask.float() - obstacle_clearance_threshold)
        return torch.sum(penalty_values, dim=1)
    ...
    def _cost_pos_limit(self):
        ...
        upper_limit[:, [3, 7, 11, 15]] = 0.0
        lower_limit[:, [3, 7, 11, 15]] = 0.0
